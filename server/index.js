...
io.on("connection", (socket) => {
    ...
    socket.on("joinChat", ({ sender, receiver }) => {
        ...
        Message.find({
            ...
            .then(async (messages) => {
                ...
                // Mark messages as seen by the sender
                await Message.updateMany(
                    { receiver: sender, sender: receiver, seenBy: { $ne: sender } },
                    { $addToSet: { seenBy: sender } }
                );
                socket.emit("loadMessages", messages);
            })
            .catch((error) => console.error("Error fetching messages:", error));
    ...
    socket.on("deleteMessages", async ({ sender, messageIds }) => {
        try {
            // Mark selected messages as deleted for the sender
            await Message.updateMany(
                { _id: { $in: messageIds } },
                { $addToSet: { deletedBy: sender } }
            );
    ...
    socket.on("markAsRead", async ({ sender, receiver }) => {
        try {
            ...
            await Message.updateMany(
                { sender, receiver, seenBy: { $ne: receiver } },
                { $addToSet: { seenBy: receiver } }
            );
            ...
            io.to(room).emit("messagesUpdated", updatedMessages);
        } catch (error) {
            console.error("Error marking messages as read:", error);
        }
    });
