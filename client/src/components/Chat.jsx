...
const ChatMessage = ({ 
    ...
    const renderFile = (file, fileUrl) => {
        // Determine the file data source
        let fileData = null;
        if (file && typeof file === "string") {
            // If file is a URL string (from backend)
            fileData = { url: file, name: file.split("/").pop() || "unknown_file" };
        } else if (file && typeof file === "object" && file.url) {
            // If file is an object { name, type, url }
            fileData = file;
        } else if (fileUrl) {
            // Fallback to fileUrl if present
            fileData = { url: fileUrl, name: fileUrl.split("/").pop() || "unknown_file" };
        }
        ...
        // Determine file type based on name or URL
        const fileName = fileData.name.toLowerCase();
        ...
        if (fileData.type) {
            ...
            fileType = "application/msword"; // or application/vnd.openxmlformats-officedocument.wordprocessingml.document for .docx
        } else if (fileName.endsWith(".ppt") || fileName.endsWith(".pptx")) {
            fileType = "application/vnd.ms-powerpoint"; // or application/vnd.openxmlformats-officedocument.presentationml.presentation for .pptx
        } else if (fileName.endsWith(".csv")) {
            fileType = "text/csv";
        } else if (fileName.endsWith(".xls") || fileName.endsWith(".xlsx")) {
            fileType = "application/vnd.ms-excel"; // or application/vnd.openxmlformats-officedocument.spreadsheetml.sheet for .xlsx
        }
