        ...
        if (data.length > 0 && allUsers.length > 0) {
          setSentUsers((prevSentUsers) => {
            ...
            data.forEach((msg) => {
              ...
              if (existingIndex === -1) {
                updatedSentUsers.unshift({ ...senderDetails, hasNewMessage: true, deleted: false });
              } else {
                const [user] = updatedSentUsers.splice(existingIndex, 1);
                updatedSentUsers.unshift({ ...user, hasNewMessage: true, deleted: user.deleted || false });
              }
            });
            localStorage.setItem(storageKey, JSON.stringify(updatedSentUsers));
            return updatedSentUsers;
      ...
      if (msgData.receiver === currentUser) {
        setSentUsers((prevSentUsers) => {
          ...
          if (existingIndex === -1) {
            ...
            const [user] = updatedSentUsers.splice(existingIndex, 1);
            updatedSentUsers.unshift({ ...senderDetails, hasNewMessage: true, deleted: user.deleted || false });
          }
