...
export default function Chat() {
    ...
    useEffect(() => {
        ...
        fetch("https://joblinker-1.onrender.com/api/v1/user/users/all")
            ...
            .then((data) => {
                ...
                const filteredUsers = usersArray.filter((user) => user.email !== currentUser);
                setAllUsers(filteredUsers);
    ...
    useEffect(() => {
        fetch(`https://joblinker-1.onrender.com/api/unread-messages/${currentUser}`)
            ...
            .then((data) => {
                ...
                if (messagesArray.length > 0 && allUsers.length > 0) {
                    setSentUsers((prevSentUsers) => {
                        let updatedSentUsers = [...prevSentUsers];
            ...
            .catch((err) => {
                ...
                setUnreadMessages([]); // Fallback to empty array on error
            });
    }, [allUsers, currentUser, storageKey]);
    ...
    const displayedUsers = [
        ...(Array.isArray(allUsers) ? allUsers.filter((user) => user.email === currentUser) : []),
        ...(Array.isArray(sentUsers) ? sentUsers.filter((user) => user.email !== currentUser) : []),
        ...(Array.isArray(allUsers) && Array.isArray(sentUsers)
            ? allUsers.filter(
                  (user) => user.email !== currentUser && !sentUsers.some((u) => u.email === user.email)
              )
            : []),
                        ...
                        {filteredUsers.length > 0 ? (
                            filteredUsers.map((user) => {
                                const unreadCount = Array.isArray(unreadMessages)
                                    ? unreadMessages.filter((msg) => msg.sender === user.email).length
                                    : 0;
                                const displayName = user.email === currentUser ? "You" : user.fullname;
                                return (
                                    <div
