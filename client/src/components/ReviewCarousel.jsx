      ...
      >
        <div className="max-w-screen-xl mx-auto px-6">
          <motion.div
            ...
            transition={{ delay: 0.5, duration: 0.5 }}
          >
            <h2 className="text-3xl sm:text-3xl lg:text-4xl font-bold mb-6 text-center tracking-tight user-select text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400 pt-3 pb-3">
              {t('Review')}
            </h2>
          </motion.div>
          <div className="flex justify-between items-center gap-3 pt-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 w-full">
              {reviews.length > 0 ? (
                reviews.slice(currentIndex, currentIndex + reviewsPerPage).map((review, index) => (
                  <div
                    key={index}
                    className="bg-gray-800 p-6 rounded-lg shadow-lg overflow-hidden break-words whitespace-normal"
                  >
                    <h3 className="text-xl font-semibold text-white italic">{review.name}</h3>
                    <div className="flex items-center mt-4">
                      {review.companyId?.name && (
                        <div className="flex items-center">
                          ...
                          <span className="text-lg text-gray-100 text-italic italic">
                            {review.companyId.name}
                          </span>
                        </div>
                      )}
                    ...
                    <div className="mt-4 flex items-center">
                      <div className="flex space-x-1">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <span
                            key={star}
                            className={`text-xl ${review.rating >= star ? 'text-yellow-500' : 'text-gray-500'}`}
                          >
                            ★
                          </span>
                        ))}
